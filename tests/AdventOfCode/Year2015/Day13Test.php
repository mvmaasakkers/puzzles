...
use App\AdventOfCode\Year2015\Day13;
...
class Day13Test extends TestCase
{
    ...
    {
        $testCases = [
            ['input' => new DefaultInput('Alice would gain 54 happiness units by sitting next to Bob.
...
David would gain 41 happiness units by sitting next to Carol.'), 'expected' => 330],
            ['input' => new DefaultInput($this->actualInput), 'expected' => 664],
        ];

        foreach ($testCases as $n => $testCase) {
            $this->assertEquals($testCase['expected'], (new Day13())->part1($testCase['input']), "Testcase #$n");
        }
    }
    ...
    {
        $testCases = [
            ['input' => new DefaultInput($this->actualInput), 'expected' => 640],
        ];

        foreach ($testCases as $n => $testCase) {
            $this->assertEquals($testCase['expected'], (new Day13())->part2($testCase['input']), "Testcase #$n");
        }
    }

    private string $actualInput = 'Alice would lose 2 happiness units by sitting next to Bob.
